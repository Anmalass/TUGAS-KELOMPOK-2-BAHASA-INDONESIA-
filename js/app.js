(() => {
  ...
  function makeCover(data) {
    ...
    inner.appendChild($("h1", null, data.title || "Judul Teks Prosedur"));
    if (data.objective) inner.appendChild($("p", "lead", data.objective));
    const meta = $("div", "meta");
    [["Kelompok", data.group], ["Kelas", data.className], ["Sekolah", data.school]].forEach(([label, value]) => {
