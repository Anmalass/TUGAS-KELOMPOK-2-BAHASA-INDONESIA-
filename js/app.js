/* =========================================================
   ...
   bagian-bagian presentasi di layar. Tidak ada isi presentasi
   yang ditulis langsung di sini — semua teks/gambar berasal
   dari JSON. Untuk mengganti tema tugas, edit content/content.json saja.

   Navigasi: semua bagian disusun vertikal dalam satu scroll
   panjang (#slide-viewport) dengan CSS scroll-snap, jadi bisa
   discroll bebas ATAU digeser (swipe) per-bagian — dua-duanya
   jalan otomatis lewat perilaku scroll bawaan browser, tanpa
   perlu kode swipe custom.
   ========================================================= */
...
(function () {
  ...
  const state = {
    slides: [],   // [{ tab, color, render: () => HTMLElement }]
    current: 0,
    sections: [], // elemen <section class="slide"> setelah dirender
  };
  ...
  function buildCoverSlide(data, hasMoreSlides) {
    ...
    if (hasMoreSlides) {
      wrap.appendChild(
        el("div", { class: "cover-scroll-cue", "aria-hidden": "true" }, [
          document.createTextNode("Geser atau scroll"),
          el("span", { class: "arrow" }),
        ])
      );
    }
  ...
  function buildStepSlide(step, index) {
  ...
  function buildSlides(data) {
    ...
    slides.push({
      tab: "Sampul",
      color: "var(--ink)",
      render: () => buildCoverSlide(data, hasMoreSlides),
    });
  ...
  function renderApp() {
    ...
    state.slides.forEach((slide, i) => {
      ...
      try {
        ...
        console.error(`Gagal merender bagian "${slide.tab}":`, err);
        content = el("div", { class: "slide-inner" }, [
          el("p", { class: "slide-label" }, [document.createTextNode(slide.tab)]),
          el("p", {}, [document.createTextNode(`Bagian ini gagal ditampilkan: ${err.message}`)]),
        ]);
      }
      ...
      const tabBtn = el(
        "button",
        {
          ...
          "aria-label": `Lompat ke bagian ${slide.tab}`,
        },
        [el("span", { class: "rail-tab-label" }, [document.createTextNode(slide.tab)])]
      );
  ...
  function setupScrollObserver() {
    ...
    const observer = new IntersectionObserver(
      ...
      { root: viewport, threshold: [0, 0.5, 1] }
    );
    state.sections.forEach((s) => observer.observe(s));
  }
  ...
  function goTo(index) {
    ...
    state.sections[index].scrollIntoView({ behavior: "smooth", block: "start" });
  }
  ...
  function bindControls() {
    ...
    document.addEventListener("keydown", (e) => {
      ...
      if (e.key === "ArrowDown" || e.key === "ArrowRight" || e.key === "PageDown") { e.preventDefault(); next(); }
  ...
  async function init() {
    try {
      ...
      if (!state.slides.length) throw new Error("content.json tidak berisi bagian apa pun.");
